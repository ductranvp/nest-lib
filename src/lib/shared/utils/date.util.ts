...
export function getIterableDateRange(from, to) {
  const start = setDayStart(new Date(from));
  const end = setDayStart(new Date(to));
  const iterable = [new Date(start)];
  let loop = new Date(start);
  while (loop < end) {
    const newDate = loop.setDate(loop.getDate() + 1);
    iterable.push(new Date(newDate));
    loop = new Date(newDate);
  }
  return iterable;
}

export function getIterableMonthRange(from, to) {
  const start = getMonthStart(new Date(from));
  const end = getMonthStart(new Date(to));
  const iterable = [new Date(start)];
  let loop = new Date(start);
  while (loop < end) {
